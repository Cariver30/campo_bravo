...
<head>
    ...
    <title>Nueva categoría · Barra creativa</title>
    ...
    <div class="max-w-3xl mx-auto py-12 px-4 sm:px-6 lg:px-0 space-y-8">
        ...
        <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-8 shadow-2xl space-y-8">
            <div class="space-y-2">
                ...
                <p class="text-white/60 text-sm">Define cómo se mostrará esta familia de bebidas tanto en el panel como en la portada.</p>
            </div>
            ...
            <form action="{{ route('cocktail-categories.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <p class="text-xs text-white/50 mt-2">Se utiliza dentro del panel y como fallback del título público.</p>
                </div>
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="cover_title" name="cover_title" value="{{ old('cover_title') }}"
                               class="block w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white.placeholder-white/40 focus:border-pink-400 focus:ring-pink-400" />
                        <p class="text-xs text-white/50 mt-2">Se muestra en los tabs y tarjetas del cover.</p>
                    </div>
                    <div>
                        ...
                        <input type="text" id="cover_subtitle" name="cover_subtitle" value="{{ old('cover_subtitle') }}"
                               class="block w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white.placeholder-white/40 focus:border-pink-400 focus:ring-pink-400" />
                        <p class="text-xs text-white/50 mt-2">Aparece debajo del título en la portada.</p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between gap-4 pt-4">
                    <p class="text-xs text-white/50">Podrás editarla más adelante sin perder contenido.</p>
                    <button type="submit"
