@php
    ...
    ])->filter(fn($option, $key) => ($option['visible'] ?? true) || $defaultView === $key)->all();
    ...
    $currentDays = collect(explode(',', $popup->repeat_days ?? ''))->filter(fn($day) => $day !== '')->map(fn($day) => (int) $day)->all();
    $selectedDays = collect(old('repeat_days', $currentDays))->map(fn ($day) => (int) $day)->all();
@endphp
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <div class="max-w-4xl mx-auto py-10 px-4">
        ...
        <div class="bg-white shadow-xl rounded-3xl p-8 space-y-6">
            <div class="space-y-1">
                ...
                <h1 class="text-3xl font-semibold">Editar pop-up</h1>
                <p class="text-sm text-slate-500">Actualiza el contenido, la vista de destino o el calendario de este anuncio.</p>
            </div>
            ...
            <form action="{{ route('admin.popups.update', $popup) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $popup->start_date) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $popup->end_date) }}" required
                               class="w-full rounded-2xl border border-slate-200 px-4 py-2.5 focus:ring-amber-400 focus:border-amber-400">
                    </div>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="active" name="active" class="w-full rounded-2xl border border-slate-200 px-4 py-2.5 focus:ring-amber-400 focus:border-amber-400">
                            <option value="1" {{ old('active', $popup->active) == 1 ? 'selected' : '' }}>Activo</option>
                            <option value="0" {{ old('active', $popup->active) == 0 ? 'selected' : '' }}>Inactivo</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    ...
                    <button type="submit" class="px-6 py-2 rounded-full bg-amber-500 text-white font-semibold hover:bg-amber-600">Actualizar pop-up</button>
                </div>
            </form>
        </div>
    </div>
