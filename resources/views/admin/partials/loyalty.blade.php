<div class="inner-panel space-y-6" id="loyalty-settings">
    ...
    <section class="feature-card">
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
            <div>
                <h4 class="text-lg font-semibold text-slate-900 mb-1">Meseros con acceso</h4>
                <p class="text-muted small">Genera usuarios con rol "mesero" y envíales la invitación para crear su contraseña.</p>
            </div>
        </div>
        <form method="POST" action="{{ route('admin.loyalty.servers.store') }}" class="row g-3 align-items-end mb-4">
            ...
            <div class="col-md-4">
                <button class="btn btn-dark w-100">Invitar mesero</button>
            </div>
        </form>
                ...
                <tbody>
                    @forelse($servers as $server)
                        <tr>
                            <td>{{ $server->name }}</td>
                            <td>{{ $server->email }}</td>
                            <td>{{ optional($server->invitation_sent_at)->diffForHumans() ?? 'N/A' }}</td>
                            <td class="text-center">
                                @php
                                    $statusLabel = $server->invitation_token
                                        ? 'Pendiente de activar'
                                        : ($server->active ? 'Activo' : 'Bloqueado');
                                    $badgeClass = $server->invitation_token
                                        ? 'text-bg-warning'
                                        : ($server->active ? 'text-bg-success' : 'text-bg-secondary');
                                @endphp
                                <span class="badge {{ $badgeClass }}">{{ $statusLabel }}</span>
                            </td>
                            <td class="text-end">
                                <div class="d-flex flex-wrap gap-2 justify-content-end">
                                    <form method="POST" action="{{ route('admin.loyalty.servers.resend', $server) }}">
                                        @csrf
                                        <button class="btn btn-sm btn-outline-secondary">Reenviar invitación</button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.loyalty.servers.toggle', $server) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button class="btn btn-sm {{ $server->active ? 'btn-outline-warning' : 'btn-outline-success' }}">
                                            {{ $server->active ? 'Bloquear' : 'Reactivar' }}
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.loyalty.servers.destroy', $server) }}" onsubmit="return confirm('Eliminar este mesero?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted">Aún no hay meseros registrados.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
