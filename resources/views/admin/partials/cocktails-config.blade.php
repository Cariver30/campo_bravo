    ...
    <form action="{{ route('admin.updateBackground') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
        ...
        <div>
            ...
            <label class="inline-flex items-center gap-2 mt-2 text-sm text-gray-700 dark:text-gray-300">
                <input type="checkbox" name="disable_background_cocktails" value="1" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                       {{ old('disable_background_cocktails', $settings->disable_background_cocktails ?? false) ? 'checked' : '' }}>
        ...
        <div>
            <label for="floating_bar_bg_cocktails" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fondo de navegación flotante</label>
            <input type="color" name="floating_bar_bg_cocktails" id="floating_bar_bg_cocktails" value="{{ $settings->floating_bar_bg_cocktails ?? '#1f2937' }}"
                   class="w-16 h-10 p-1 border rounded-md">
        </div>

        <div>
            <label for="floating_bar_button_color_cocktails" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Botones flotantes</label>
            <input type="color" name="floating_bar_button_color_cocktails" id="floating_bar_button_color_cocktails" value="{{ $settings->floating_bar_button_color_cocktails ?? '#000000' }}"
                   class="w-16 h-10 p-1 border rounded-md">
        </div>
