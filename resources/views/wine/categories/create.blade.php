...
<head>
    ...
    <title>Nueva categoría · Cava de vinos</title>
    ...
    <div class="max-w-3xl mx-auto py-12 px-4 sm:px-6 lg:px-0 space-y-8">
        ...
        <div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-8 shadow-2xl space-y-8">
            <div class="space-y-2">
                ...
                <p class="text-white/60 text-sm">Agrupa etiquetas y controla cómo se presentan dentro de la experiencia de vinos.</p>
            </div>
            ...
            <form action="{{ route('wine-categories.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <p class="text-xs text-white/50 mt-2">Se usa en el panel y como fallback del título público.</p>
                </div>
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="cover_title" name="cover_title" value="{{ old('cover_title') }}"
                               class="block w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 focus:border-amber-400 focus:ring-amber-400" />
                        <p class="text-xs text-white/50 mt-2">Aparece en tabs y tarjetas del cover.</p>
                    </div>
                    <div>
                        ...
                        <input type="text" id="cover_subtitle" name="cover_subtitle" value="{{ old('cover_subtitle') }}"
                               class="block w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-white/40 focus:border-amber-400 focus:ring-amber-400" />
                        <p class="text-xs text-white/50 mt-2">Texto complementario para la portada.</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                    ...
                    <input type="checkbox" id="show_on_cover" name="show_on_cover" value="1" {{ old('show_on_cover') ? 'checked' : '' }}
                    ...
                    <label for="show_on_cover" class="text-sm text-white/80">
                        Mostrar esta categoría como pestaña en la portada
                    </label>
                </div>

                <div class="flex items-center justify-between gap-4 pt-4">
                    <p class="text-xs text-white/50">Podrás editarla más adelante sin perder el orden.</p>
                    <button type="submit"
